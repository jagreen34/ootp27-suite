...
# players and the pass/fail flag cannot tell them apart. At the premium
# positions one grade of glove is worth roughly a whole grade of power, which
# is rule 12's "you can pay for it" band — so the MARGIN gets reported, and a
# big margin can carry a bat that fails its own bars.
# Source: the TIEBREAK ladder below; CF range is the largest defensive
# coefficient measured in the league (+0.764/pt, t=13.9) [A88 sec.3].
GLOVE_GRADE_WRC = {'CF': 9.9, 'SS': 10.1, '2B': 10.4, '3B': 6.9}
GLOVE_CARRIES = 5                      # one full grade clear of the gate

# ⚠⚠ THE wRC+ FIGURE IS LINEAR AND THE CURVE ABOVE 70 IS UNMEASURED.
# A83 sec.13/14 reported range saturating above 70. A88 sec.4 tried to settle it
# within-player and COULD NOT: defensive ratings almost never change season to
# season (A50/A58d: 92% unchanged), so there are too few movers to fit a curve.
# It stays observational and advisory. Multiplying a per-grade constant by two
# grades therefore EXTRAPOLATES into exactly the region in doubt, and a 70 is
# the first rating where that bites. Flag it; do not quietly bank it.
SATURATION_WATCH = 70
...
def rate(row, key, denom_key):
    ...
    stat under-reports and is at least obviously wrong. A silent zero on a
...
def grade_bat(r, name):
    ...
    # glove
    margin = None                       # display points clear of the gate
    raw_rng = None
    if pos in RANGE_GATE:
        col = 'IF RNG' if pos in ('SS', '2B') else 'OF RNG'
        v = num(r, col, name)
        raw_rng = v
        margin = v - RANGE_GATE[pos]
        glove = ('PASS' if margin >= 0 else 'FAIL',
                 '%s %.0f v %d (%+.0f)' % (col, v, RANGE_GATE[pos], margin))
    elif pos == '3B':
        s = num(r, 'IF RNG', name) + num(r, 'IF ARM', name)
        raw_rng = num(r, 'IF RNG', name)
        margin = (s - GATE_3B_SUM) / 2.0        # two ratings, so halve to grade units
        glove = ('PASS' if s >= GATE_3B_SUM else 'FAIL',
                 'rng+arm %.0f v %d (%+.0f)' % (s, GATE_3B_SUM, s - GATE_3B_SUM))
    elif pos in NO_GATE:
        glove = ('n/a', 'no gate')
    else:
        glove = ('?', 'unknown pos %s' % pos)
    worth = (GLOVE_GRADE_WRC.get(pos, 0) * margin / 5.0) if margin is not None else None
    if worth is not None and raw_rng is not None and raw_rng >= SATURATION_WATCH:
        glove = (glove[0], glove[1] + ' ~sat?')     # linear past 70 is unmeasured
    return {'side': 'BAT', 'tools': len(cleared), 'bars': '+'.join(cleared) or '-',
            'lead_tool': 'yes' if lead else 'NO',
            'glove': glove[0], 'glove_detail': glove[1],
            'glove_wRC': '' if worth is None else '%+.0f' % worth,
            'verdict': bat_verdict(len(cleared), lead, glove[0], pos, margin, worth)}

def bat_verdict(n, lead, glove, pos=None, margin=None, worth=None):
    """⚠ The glove enters here as a MAGNITUDE, not just a gate.

    Before this, a centre fielder a full grade clear of the gate and one sitting
    exactly on it both printed 'PASS', and a light bat behind either of them
    printed 'below a no-tool bat' — a statement about his BAT rendered as his
    VALUE. At CF one grade of range is 9.9 wRC+, which rule 12 puts in the
    pay-for-it band, so it can outweigh a missing bar. [A88 sec.3 / TIEBREAK]
    """
    if glove == 'FAIL':
        return 'GLOVE FAILS THE GATE'
    carry = (margin is not None and margin >= GLOVE_CARRIES
             and pos in GLOVE_GRADE_WRC)
    tag = '' if not carry else '  [glove %+.0f wRC+]' % worth
    if n == 0:
        return ('GLOVE-FIRST REGULAR — no bat, but the glove pays' + tag) if carry \
               else 'no top-quartile tool'
    if not lead:
        return ('GLOVE-FIRST REGULAR — bat fails rule 2, glove carries him' + tag) if carry \
               else 'RULE 2: no power or eye — below a no-tool bat'
    if n >= 3:
        return 'core bat' + tag
    if n == 2:
        return 'qualifying bat' + tag
    return ('one-tool bat, PLUS GLOVE' + tag) if carry \
           else 'one-tool: playable, not a building block'
...
def grade_arm(r, name):
    ...
    return {'side': 'ARM', 'eff': eff, 'role': role, 'HRA': hra, 'STU': stu,
            ...
            'verdict': arm_verdict(hra, stu, eff, ctl, stm),
            'detail': '; '.join(flags) if flags else '-'}

def arm_verdict(hra, stu, eff, ctl, stm):
    """The bats' ladder, for arms. Tested in AXIS ORDER, not column order: the
    primary axis decides first and a failure there is not offset by anything
    below it. A 4-pitch arm with light HRA is a BAD starter, and the old flag
    list rendered that identically to 'stamina short'.

    ⚠ The HRA-light rung CARRIES THE STUFF NUMBER, and that is not decoration.
    On this roster 12 of 19 arms fail HRA, so without it the column stops
    discriminating exactly where most of the staff lives — stuff 60 with three
    pitches reads the same as stuff 25 with one.

    ⚠ AND IT IS DELIBERATELY NOT A RANKING. It would be easy to weight the axes
    by A88's coefficients and sort. Do not. Those are WITHIN-PLAYER change-on-
    change estimates; using them to rank LEVELS across different pitchers assumes
    the same slope applies to levels, which A88 does not establish — and A88 sec.0
    is specifically about how cross-sectional pitcher estimates go wrong. Show
    both numbers, let the GM weigh them.
    """
    if hra >= ACE_HRA and ctl >= ACE_CTL:
        return 'ACE — clears the league\'s top gate'          # A86 sec.4
    if hra < HRA_OK:
        s = 'HRA LIGHT (stuff %s' % ('?' if stu is None else '%.0f' % stu)
        if stu is not None and eff >= STARTER_EFF:
            s += ', %d pitches' % eff                        # the two that travel together
        return s + ')'
    if eff < STARTER_EFF:
...
def main():
    ...
    for r in rows:
        ...
        g.update({'Name': name, 'POS': r['POS'], 'Age': r.get('Age', ''),
                  ...
                  'wRC+': rate(r, 'wRC+', 'PA'), 'FIP-': rate(r, 'FIP-', 'IP')})
        out.append(g)
