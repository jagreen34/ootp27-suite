...
def _staff_upgrade_ui(incumbents, staff_all, projected, role, slots, key):
    ...
    ups, displaced, floor = find_staff_upgrades(
        cand, staff_all, projected, role, slots,
...
with tab_lineup:
    ...
    if cand_file is not None:
        ...
        if cand_scope.startswith("Free"):
            ocol = next((c for c in ("ORG", "TM", "Team", "Org") if c in cand.columns), None)
            if ocol is None:
                st.warning("No ORG/TM/Team column in that file — cannot isolate free "
                           "agents, so everyone is being considered.")
            else:
                fa = cand[ocol].astype(str).str.strip().isin(["-", "", "nan", "NaN", "None"])
                cand = cand[fa]
                st.caption(f"Free-agent filter: **{len(cand):,} of {n_all:,}** rows have "
                           f"an empty `{ocol}`.")
                if not len(cand):
                    st.info("No free agents in that file. Either it is a roster export "
