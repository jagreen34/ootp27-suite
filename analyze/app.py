...
Every upload runs the 57-check self-test FIRST.  If a check fails the page says so
...
<div class="card">
  ...
  <ul style="margin-top:12px">
    <li>A league file of ~1,500 players takes about <b>90 seconds</b>. A draft pool is instant.</li>
...
if(i){
  ...
  f.onsubmit=()=>{ b.disabled=true; s.classList.add('on');
     ...
     st.textContent = big ? 'Working — a league file takes about 90 seconds…'
                          : 'Working — running 57 self-tests, then the board…';
...
def analyze():
    ...
    with tempfile.TemporaryDirectory() as td:
        ...
        try:
            ...
            fr = T.Frame(rows, src, orig_name=os.path.basename(up.filename))
            ...
            if not players:
                return Response(
                    ...
                    mimetype='text/plain', status=400)
            notes = [f"detected {fr.kind} export (G9)."]
            if thin:
