...
VERSION = "1.7.0  (registry v16, A1-A115)"
...
MIN_CHECKS = 283   # checks run BEFORE this one; the suite reports 284
SEED    = 1977
...
NO_POTENTIAL_IN_OOTP = frozenset({'BABIP'})
...
def build(fr, rng=None):
    ...
    for i, r in enumerate(fr.rows):
        ...
        if arm:
            ...
            # v1.5.0 switched only pos_runs_600 and left pos_runs / pos_war on the
            # gate, so the BATS sheet carried three columns labelled 'pos' on one row
            # disagreeing by 13.94 runs.  There is now no second path to get wrong.
...
def write_workbook(fr, players, out_path, extra_notes=()):
    ...
    hb = ['Name','POS','lands','Age','Class','ORG','OVR*','POT*',
          ...
          'priced at','bat runs/600 (NO pos)','+/- 1 SE','pos runs/600 (A74)','total runs/600',
          'bat score note','pos runs/season','pos career WAR']
    ...
    for p in sorted(bats, key=lambda x: -(x.get('ewar_adj') or 0)):
        row = [p['name'],p['pos'],p['lands'],p['age'],p['cls'],p['org'],p['ovr_star'],p['pot_star'],
               ...
               p['bat_note'],p['pos_runs'],p['pos_war']]
    ...
    if not pool:
        ...
        hr = ['#','pool','slot','Name','ORG','POS','lands','Age','OVR*','POT*',
              'IF RNG','IF ARM','OF RNG','OF ARM','TDP','C ABI',
              'PA / IP','wRC+ / FIP-','WAR',
              'bat runs/600 (NO pos)','+/- 1 SE','pos runs/600 (A74)','total runs/600',
              'bat score note',
...
def verify(verbose=True):
    ...
    c.true("the ESTIMATED ones are exactly the four we know are assumptions",
           ...
           {'SHARE','RUNS_PER_WIN','SEASONS_DEFAULT','BAT_RUNS_MIN_PA'},
    ...
    def _row(name, pos, org, wrc, pa, war, ifr=70, ofr=70,
             pow_='50', eye='50', age='27', powp=None, eyep=None):
    ...
    c.eq("G16: an everyday SS whose glove misses A108's bar still lands 1B (acquisition)",
         1.0 if _cell('Everyday SS', 'lands') == '1B' else 0.0, 1.0, 0)
    ...
    c.eq("bat + pos = total, read off the written cells",
         ...
         _cell('Real 1B', 'total runs/600'), 0.01)
    c.eq("and the two men differ in the TOTAL by exactly the positional gap",
         _cell('Everyday SS', 'total runs/600') - _cell('Real 1B', 'total runs/600'),
    ...
    c.eq("... and so does 'pos runs/season' -- one key, every column",
         _bcell('Everyday SS', 'pos runs/season'), pos_adj_runs('SS'), 0.01)
    c.eq("... and so does 'pos career WAR'",
         _bcell('Everyday SS', 'pos career WAR'), pos_adj_war('SS'), 0.01)
    ...
    c.true("and carries NO positional runs at all -- a pitcher priced at 1B is -10.87",
           ...
           and _cell('Ace', 'total runs/600') is None
    ...
    _ALLOWED_EMPTY_POOL = {
        ...
        'total runs/600':        'G7 on a pool: no bat half to add to the position half',
        'bat score note':        'G7 on a pool',
    }
    _PCT  = {'P(POW55)','P(OBP55)','P(both)','P(ace)','P(#1)','P(#2)','ceiling %'}
    _NUMS = {'bat runs/600 (NO pos)','pos runs/600 (A74)','total runs/600','+/- 1 SE',
