"""
OOTP 27 Suite — Development Layer 3: Reserve-Roster Keep-or-Cut Allocator
=========================================================================
The "real prize." Jeff runs NO minor leagues — a reserve roster + 40-man. Every
developing prospect occupies a scarce seat a backup/swingman could hold. This
ranks the three claimants for reserve seats and recommends keep/cut BY MODE.

Claimants (brief):
  • prospect          — future value; depreciating defensive clock (A19)
  • backup            — present insurance
  • borderline starter— present value or trade chip
...
Phase-aware seat caps (Jeff's league rules):
  Opening Day   25 active / 15 reserve   (binding cut; picks NOT protected)
  Sept 1 → EOS  40 active / (rest)
  Aug 31 → EOS  25 active / 15 + newest picks   (picks protected)
  Offseason     — / 15 + newest picks           (picks protected)
  Spring        40 active / 60 reserve          (picks protected)

Pure logic only — the renderer lives in development.py and reuses its
click-to-inspect modal. No Streamlit imports here.
"""
...
# Editable in development_state['reserve'].
MODE_WEIGHTS = {
