...
class Entry:
    ...
    def label(self, show_kind: bool = False) -> str:
        head = f"{self.kind} · " if show_kind else ""
        return f"{head}{self.orig}  ·  {self.when}  ·  {self.kb} KB"
...
def picker(st, label: str, key: str, kind: str,
           ...
           also: tuple[str, ...] = (), default_to_saved: bool = True):
    ...
    upload, saved files are offered — this kind first, then the kinds named in
    `also`, tagged so you can see what you are picking.

    ⚠ `also` exists because the same physical export serves several slots: the
    league file you rank against is also the candidate pool the optimizer
    searches. Filing them under separate kinds meant a file cached in one tool
    was invisible in the other, which reads as "it didn't save."

    ⚠ Only a file of the PRIMARY kind is ever pre-selected. An `also` file must
    be chosen deliberately — silently loading a roster into a candidate-pool
    slot would be worse than not remembering it at all.
    """
    ...
    primary = available(kind)
    seen = {e.path for e in primary}
    extra = []
    for k in also:
        for e in available(k):
    ...
    # Tag every row with its kind whenever more than one kind is on offer, so
    # picking a league file into a pool slot is a visible choice.
    tag = bool(extra)
    opts = ["(none)"] + [e.label(show_kind=tag) for e in saved]
    idx = 1 if (default_to_saved and primary) else 0
    pick = st.sidebar.selectbox(f"…or reuse a saved file", opts, index=idx,
                                key=f"{key}__saved")
    if pick == "(none)":
        return None
