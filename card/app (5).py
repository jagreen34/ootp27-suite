    ...
    if rows:
        ...
        for r in sel:
            ...
            try:
                ...
                continue
            # rate() not .get() — a rate stat with no playing time behind it is
            # unknown, not zero, and FIP- 0 is a PERFECT score. See quakers.rate.
            g.update({"Name": name, "POS": r.get("POS"), "Age": r.get("Age", ""),
                      "PA": r.get("PA", ""), "IP": r.get("IP", ""),
                      "wRC+": CARD.rate(r, "wRC+", "PA"),
                      "FIP-": CARD.rate(r, "FIP-", "IP")})
        ...
        st.caption("**wRC+ and FIP− are outcomes, not ratings — and both are already park- and "
                   "league-adjusted (A106).** Do not re-adjust a Quakers player for the ballpark; "
                   "that double-counts, and FIP− over-credits an HR-heavy park by ~2 points anyway. "
                   "A blank means no playing time, never a zero.")

        st.subheader("Bats")
        st.caption("Bars: " + " · ".join("%s ≥ %d" % (k, v) for k, v in CARD.BARS_AC.items())
                   + "  — AC percentiles. Do not use parquet bars on AC players.")
        st.caption("**`glove_wRC` is a RATINGS estimate, not a measurement — read it as a flag "
                   "to look, never a number to bank.** It prices the margin over the gate at "
                   "the tiebreak ladder's per-grade value, because a gate is not a valuation: "
                   "clearing by 5 and clearing by 25 are different players. Two caveats. "
                   "`~sat?` marks a range rating of %d+, where A83 saw saturation and A88 could "
                   "NOT test it (defensive ratings are 92%% unchanged year to year, so there are "
                   "too few movers to fit a curve) — those figures extrapolate. And a rating is "
                   "not a season: check the player's actual ZR before trusting either."
                   % CARD.SATURATION_WATCH)
        st.dataframe(sorted(bats, key=lambda x: (-x["tools"], x["glove"] == "FAIL")),
                     use_container_width=True, hide_index=True,
                     column_order=["Name", "POS", "Age", "PA", "tools", "bars", "lead_tool",
                                   "glove", "glove_detail", "glove_wRC", "verdict", "wRC+"])
