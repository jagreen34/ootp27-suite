...
from dev_model import (
    ...
    effective_position, role_of,
    playable_positions, park_delta,
    # ── v15.45 ──────────────────────────────────────────────────────────────
...
league_file = store.picker(
    st, "League population (sets the bars)", key="lg", kind="league",
    also=("pool", "roster"),
    help="Saved to the shared volume and offered back next time. /lineup/ "
         "sees the same files — upload once.")
target_file = store.picker(
    st, "Players to rank (roster / draft pool)", key="tg", kind="roster",
    also=("league", "pool"),
    help="Shares the 'roster' slot with /lineup/, so a roster uploaded there "
         "shows up here.")
...
nav.render(st, "rank", note=f"registry {K.REGISTRY_VERSION}")
st.title("📊 OOTP Player Rank")
...
def org_of(r):
    ...
    ⚠ A literal "-" means FREE AGENT -- it is DATA, not a missing value.
    Returning "" for it collapsed 416 free agents into blanks that the Teams
    filter then dropped entirely, so there was no way to select them. That is
    the silent-zero failure [methodology rule 22]: a real category rendered as
    absence. "-" now maps to a distinct, filterable label; genuinely absent
    org data still returns "".
    """
    for c in ("ORG", "TM", "Team", "Org", "Organization"):
        ...
        if sv in ("nan", ""):
            continue
        return FREE_AGENT if sv == "-" else sv
    return ""
...
match_tools = st.checkbox(
    ...
    help="Eligibility in OOTP is experience, not ability — the tools are the "
         ...
         "never played there.")
_orgs = [o for o in d.get("Org", pd.Series(dtype=str)).unique() if str(o).strip()]
_fa_n = int((d.get("Org", pd.Series(dtype=str)) == FREE_AGENT).sum())
# Free agents first -- they are the most-used filter and the easiest to miss
# at the bottom of a 28-team alphabetical list.
orgs = ([FREE_AGENT] if FREE_AGENT in _orgs else []) + sorted(o for o in _orgs if o != FREE_AGENT)
org_filter = st.multiselect(
    "Teams", orgs,
    help=f"Blank = every team. **{FREE_AGENT}** is the export's \"-\" org "
         f"({_fa_n} players here) — unsigned players you could simply sign.") if orgs else []
