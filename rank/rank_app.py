...
bars, league_scores = positional_bars(league_df)
...
# ── batters ───────────────────────────────────────────────────────────────────
def build_batters(df):
    rows = []
    for _, r in df.iterrows():
        if str(r.get("RL", "-")) != "-":
            continue
        if show_projected:
            proj = project_batter(r)
        else:
            proj = {t: pd.to_numeric(r.get(c), errors="coerce")
                    for t, (c, _) in
                    {"POW": ("POW", None), "EYE": ("EYE", None),
                     "BABIP": ("BABIP", None), "GAP": ("GAP", None),
                     "AVK": ("K's", None)}.items()}
        sc = score_batter(proj)
        if not sc:
            continue
        p0 = str(r.get("POS")).split("/")[0].upper()
        bar = bars.get(p0)
        pos_pool = None
        if p0 in POSITIONS:
            m = league_df[league_df["POS"].astype(str).str.split("/").str[0]
                          .str.upper() == p0]
            if len(m):
                pos_pool = pd.Series([
                    score_batter(project_batter(x) if show_projected else
                                 {t: pd.to_numeric(x.get(c), errors="coerce")
                                  for t, c in [("POW", "POW"), ("EYE", "EYE"),
                                               ("BABIP", "BABIP"), ("GAP", "GAP"),
                                               ("AVK", "K's")]})
        ...
        rows.append({
            ...
            "Score": sc,
            "Pct@POS": pct(pos_pool, sc) if pos_pool is not None else np.nan,
            "PctAll": pct(pd.Series(league_scores), sc),
...
def build_pitchers(df):
    ...
    d = pd.DataFrame(rows)
    if len(d):
        pool = d["Score"] if target_file is None else pd.Series(
            [score_pitcher(project_pitcher(x) if show_projected else {
             ...
             for _, x in league_df[league_df["RL"] != "-"].iterrows()])
        d["PctAll"] = d["Score"].apply(lambda s: pct(pool, s))
    return d


with st.spinner("Scoring…"):
    d = build_batters(target_df) if mode == "Batters" else build_pitchers(target_df)

if not len(d):
    st.warning("No rows of that type in the uploaded file.")
    st.stop()
...
cols = (["Name", "POS", "Age", "Score", "Pct@POS", "PctAll", "vsBar",
         "Budget", "POW", "EYE", "HT", "GAP", "AVK"]
        if mode == "Batters" else
        ["Name", "POS", "Age", "Score", "PctAll", "STU", "MOV", "CON", "RealPitches"])
