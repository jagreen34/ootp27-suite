...
import roster_construction as rc   # shared reserve keep/cut primitive (was reserve_roster)
...
DEV_DEFAULTS = {
    ...
    'reserve':           None,           # populated from rc.RESERVE_DEFAULTS in _load
}
...
def _load_dev_state(league: League) -> dict:
    ...
    merged['reserve'] = {**rc.RESERVE_DEFAULTS, **rsv}
    merged['reserve']['class'] = {**rc.CLASS_DEFAULTS, **(rsv.get('class') or {})}
    return merged
...
def render_development(league: League):
    ...
    tabs = st.tabs(["🎚️ Slider Optimizer", "📐 Methodology"])
    with tabs[0]:
    ...
    st.caption("ℹ️ Reserve keep/cut moved to the **🧱 Roster Construction** section "
               "(it's a cross-cutting roster primitive, not a Development concern). "
               "It still consumes Layer-1 ΣweightedGap from this optimizer.")
